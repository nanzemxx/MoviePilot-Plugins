...
class SmartSignIn(_PluginBase):
    ...
    def __signin_base(site_info: CommentedMap) -> Tuple[bool, str]:
        ...
        try:
            # 导入 CloudFlare 绕过工具
            from .cf_bypass_helper import CloudFlareBypassHelper
            ...
            # 使用智能 HTTP 客户端 (CloudFlare 绕过)
            cf_bypass = CloudFlareBypassHelper(proxies=proxies, verify_ssl=False)
            
            try:
                # 使用预热机制访问签到页面
                response = cf_bypass.warmup_and_get(
                    url=checkin_url,
                    cookies=site_cookie,
                ...
                # 判断登录状态
                if not SiteUtils.is_logged_in(response.text):
                    if under_challenge(response.text):
                        logger.warn(f"{site} 签到失败，站点被Cloudflare防护 (智能客户端仍被拦截)")
                        return False, "签到失败，Cloudflare防护拦截！"
                    elif response.status_code == 200:
                        logger.warn(f"{site} 签到失败，Cookie已失效")
                        return False, "签到失败，Cookie已失效！"
                    else:
                        logger.warn(f"{site} 签到失败，状态码：{response.status_code}")
                        return False, f"签到失败，状态码：{response.status_code}！"
                else:
                    # 已登录，判断是否已签到
                    if re.search(r'已签|签到已得', response.text, re.IGNORECASE) or SiteUtils.is_checkin(response.text):
                ...
                # 尝试访问站点首页作为兜底
                logger.info(f"{site} 签到页面访问失败，尝试访问首页: {str(e)}")
                if site_url != checkin_url:
                    response = cf_bypass.smart_get(
                        url=site_url,
                        cookies=site_cookie,
                        headers={'User-Agent': ua} if ua else None,
                        timeout=30
                    ...
                    if response and response.status_code in [200, 500, 403]:
                        if not SiteUtils.is_logged_in(response.text):
                            if under_challenge(response.text):
                                msg = "站点被Cloudflare防护 (智能客户端仍被拦截)"
                            elif response.status_code == 200:
                                msg = "Cookie已失效"
                            else:
                            ...
                            logger.info(f"{site} 访问首页成功（签到状态未知）")
                            return True, "访问成功"
                    else:
                        raise
                else:
    ...
    def __login_base(site_info: CommentedMap) -> Tuple[bool, str]:
        ...
        try:
            ...
            if response and response.status_code in [200, 500, 403]:
                ...
                logger.warn(f"{site} 模拟登录失败，状态码：{response.status_code}")
